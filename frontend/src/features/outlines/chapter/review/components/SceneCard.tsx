import type { ChapterOutlineScene } from "../../../../../data/mockData";
import { InlineComment } from "./InlineComment";

export interface SceneCardProps extends Readonly<{
  scene: ChapterOutlineScene;
}> {}
...
export function SceneCard({ scene }: SceneCardProps) {
  return (
    <article className="relative rounded-lg border border-surface-variant bg-surface-container-lowest p-6 shadow-[0_4px_24px_rgba(48,49,46,0.03)] [content-visibility:auto] [contain-intrinsic-size:auto_300px]">
      ...
      />
      <h4 className="mb-4 font-label text-xs uppercase tracking-[0.18em] text-on-surface-variant">
        {scene.id}
      </h4>
      ...
      {scene.comment ? <InlineComment text={scene.comment} /> : null}
    </article>
  );
}
